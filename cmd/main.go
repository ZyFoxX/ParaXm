...
func printUsage() {
	...
	fmt.Print("Options:\n")
	fmt.Print("  -u string      Target URL to scan (e.g., https://example.com)\n")
	fmt.Print("  -o string      Output file to save results\n")
	fmt.Print("  -d int         Crawling depth (default: 2)\n")
	fmt.Print("  -t int         Number of concurrent threads (default: 10)\n")
	fmt.Print("  -timeout int   Request timeout in seconds (default: 10)\n")
	fmt.Print("  -p string      Proxy URL (e.g., http://127.0.0.1:8080)\n")
	fmt.Print("  -r int         Max retries for failed requests (default: 2)\n")
	fmt.Print("  -f bool        Follow redirects (true/false) (default: true)\n")
	fmt.Print("  -ratelimit float Maximum requests per second (0 for unlimited) (default: 0)\n")
